...
public class ResolvedServiceCatalog {
    ...
    public ResolvedServiceCatalog(EmulatorConfig config) {
        this.catalog = new ServiceCatalog(List.of(
                ...
                // Rhombus fork: Rekognition stub. No config knob in the upstream
                // schema; hardcode enabled=true since the handler is a no-op
                // stub registered directly in AwsJson11Controller.
                descriptor("rekognition", "rekognition", true, true,
                        null, null, 5000L, null, ServiceProtocol.JSON,
                        protocols(ServiceProtocol.JSON),
