...
final class ExpressionEvaluator {
    ...
    private static boolean evaluateFunction(FunctionCallExpr func, JsonNode item,
        ...
        return switch (funcLower) {
            ...
            // Rhombus fork: attribute_type(path, type) — checks the AttributeValue
            // wire-type tag of an attribute (S, N, B, BOOL, NULL, L, M, SS, NS, BS).
            // Carried over from c2d414a; upstream's evaluator covered all other
            // function calls but not this one.
            case "attribute_type" -> {
                if (func.args().size() < 2) yield false;
                String path = resolveAttributePath(func.args().get(0), exprAttrNames);
                JsonNode attrValue = item != null ? resolveNestedAttribute(item, path) : null;
                if (attrValue == null) yield false;
                String expectedType = resolveScalar(func.args().get(1), item, exprAttrNames, exprAttrValues);
                yield expectedType != null && attrValue.has(expectedType);
            }
            default -> false;
        };
    }
